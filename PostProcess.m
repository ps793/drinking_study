...
% for each day
    ...
    %%%%%%%%%%%%%%weekend %%%no necessary on the weekend for now
    % night 18-3
    ...
    for r=1:size(pos_block,1)
        ...
        if pos_block{r,4}==1
            ...
            data_length=[];
            for c=1:3
                if pos_block{r,c}.rr(1)~=99999
                    pos_data=[pos_data;pos_block{r,c}];
                    data_length(c)=size(pos_block{r,c},1);
                end
            end
            if isempty(pos_data)
                neg_block{r,3}=[];
            ...
            if pos.rr(1)~=99999
                if pos_block{r,1}.rr(1)==99999
                    pos=[pos_block{r,1};pos];
                    data_length(1)=size(pos_block{r,1},1);
                end
                if pos_block{r,2}.rr(1)==99999
                    pos=[pos;pos_block{r,2}];
                    data_length(2)=size(pos_block{r,2},1);
                end
                pos_len=length(pos);
                ...
                while isempty(neg_block{r,1})
                    ...
                    while n_idx <= length(closer_data)-pos_len+1
                        ...
                        %%%%%we can update pos_len variable....
                        %%%%for 1,2 len>=45min, for 3, any
                        if n_idx==length(closer_data)-pos_len+1
                            disp(pos_len);
                            n_idx=1;
                            if sum(data_length(1:2))>60*45+2                                
                                pos_len=max(60*45+2,pos_len-15*60);
                            else
                                pos_len=length(pos);
                            end
                            pos_time_interval=datenum(pos.datetime(pos_len,:))-datenum(pos.datetime(1,:));
                        end;
                        next=closer_data(n_idx:n_idx+pos_len-1,:);
                        next(next(:,5)>=24,5)=next(next(:,5)>=24,5)-24;
                        non_next=next;
                        non_next(isnan(non_next(:,8)),:)=[];%interpolation%%jiaxianzhi
                        if ~isempty(non_next)
                        ...
                        if length(non_next)==length(next) && abs(abs(neg_time_interval)-pos_time_interval)<=0.001 %%need to be changed
                            standard=datenum(60*15/3600/24);
                            interval=datenum(pos(sum(data_length(1:2)),2).datetime)-datenum(pos(1,2).datetime);
                            next_interval=datenum(next(end,2:7))-datenum(next(1,2:7));
                            if next_interval>interval
                                interval1=datenum(pos(data_length(1),2).datetime)-datenum(pos(1,2).datetime);
                                interval2=datenum(pos(sum(data_length(1:2)),2).datetime)-datenum(pos(data_length(1)+1,2).datetime);
                                
                                s=1;
                                for time_idx=1:size(next,1)
                                    if datenum(next(time_idx,2:7))-datenum(next(s,2:7))>interval1
                                        neg_block{r,1}=next(s:time_idx,:);
                                        s=time_idx;%reverse one step. incase of big junk
                                        break;
                                    end
                                end
                                if s==size(next,1)
                                    neg_block{r,2}=next(s,:);
                                    neg_block{r,3}=[];
                                    neg_block{r,4}=1;                                 
                                end
                                for time_idx=s:size(next,1)
                                    if datenum(next(time_idx,2:7))-datenum(next(s,2:7))>=interval2
                                        neg_block{r,2}=next(s:time_idx,:);
                                        neg_block{r,3}=next(time_idx+1:end,:);
                                        neg_block{r,4}=1;
                                        break;
                                    end
                                end
                            ...
                            if next_interval>=standard && next_interval<interval
                                interval1=datenum(pos(data_length(1),2).datetime)-datenum(pos(1,2).datetime);
                                interval2=datenum(pos(sum(data_length(1:2)),2).datetime)-datenum(pos(data_length(1)+1,2).datetime);
                                ...
                                neg_block{r,2}=next(s:end,:);
                                neg_block{r,3}=[];
                                neg_block{r,4}=1;
                            end
                            if next_interval<standard
                                interval1=datenum(pos(data_length(1),2).datetime)-datenum(pos(1,2).datetime);
                                interval2=datenum(pos(sum(data_length(1:2)),2).datetime)-datenum(pos(data_length(1)+1,2).datetime);
        ...
        if pos_block{r,4}==1
            ...
            for c=1:3
                ...
                if ~isempty(pos)
                    ...
                    one_output=[s_time,e_time,type,is_drink,dosage,hr,ibi];
                    pos_combine=[pos_combine,one_output];
                else
                    nine_output=repmat(99999,1,17);
    ...
    output_ep.Properties.VariableNames={'user','episode','s_year_1','s_month_1','s_day_1','s_hour_1','s_min_1','s_sec_1',...
                                        'e_year_1','e_month_1','e_day_1','e_hour_1','e_min_1','e_sec_1','type_1','is_drinking_1','dosage_1','HR_1','RR_1',...
                                        's_year_2','s_month_2','s_day_2','s_hour_2','s_min_2','s_sec_2',...
                                        'e_year_2','e_month_2','e_day_2','e_hour_2','e_min_2','e_sec_2','type_2','is_drinking_2','dosage_2','HR_2','RR_2',...
                                        's_year_3','s_month_3','s_day_3','s_hour_3','s_min_3','s_sec_3',...
                                        'e_year_3','e_month_3','e_day_3','e_hour_3','e_min_3','e_sec_3','type_3','is_drinking_3','dosage_3','HR_3','RR_3',...
